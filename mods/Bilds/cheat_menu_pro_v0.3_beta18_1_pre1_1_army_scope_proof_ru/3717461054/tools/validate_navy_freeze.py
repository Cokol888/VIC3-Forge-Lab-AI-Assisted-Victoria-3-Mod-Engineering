...
def _balanced_named_blocks(text:str):
    """Return selected Navy workspace blocks without depending on surrounding post-Final UI."""
    pattern=re.compile(r'(?:widget|scrollarea)\s*=\s*\{\s*name\s*=\s*"([^"]+)"')
    selected=[]
    for match in pattern.finditer(text):
        name=match.group(1)
        keep=(name=='cmp_workspace_fleet_picker' or
              re.fullmatch(r'cmp_workspace_navy18_(catalog|new|existing|shipctrl|transfer|logistics)_(compact|standard|large|xlarge)',name))
        if not keep:
            continue
        ...
        while i<len(text):
            ...
            if in_str:
                ...
                if c=='"': in_str=True
                    ...
                    depth-=1
                    if depth==0:
                        selected.append((name,normalize_jomini(text[match.start():i+1])))
                        break
            i+=1
        else:
            raise ValueError('unclosed workspace block '+name)
    selected.sort(key=lambda item:(item[0],item[1]))
    return selected

def workspace_navy_surface_hash(text:str):
    blocks=_balanced_named_blocks(text)
    payload='\n'.join(name+'='+body for name,body in blocks)
    return len(blocks),sha(payload.encode('utf-8'))

def main()->int:
    errors=[]; warnings=[]; checks={}
    build=load('registry/build.json'); navy=load('registry/navy18.json'); cat=load('registry/naval_catalog.json'); freeze=load('registry/beta18_final_freeze.json'); postfreeze=load('registry/beta18_postfinal_navy_freeze.json') if (ROOT/'registry/beta18_postfinal_navy_freeze.json').exists() else None
    # Post-Final builds may change top-level build identity, but must descend from
    # the released beta18 Final and retain its frozen Navy gameplay surface.
    if not str(build.get('baseline_victoria','')).startswith('1.13'): errors.append('Victoria baseline changed outside 1.13 after beta18 Final')
    if build.get('fleet_gate') not in ['BETA18_RC1_FULL_RUNTIME_PASS; NAVY_REWORK_RELEASED','BETA18_FINAL_RUNTIME_PASS; NAVY_REWORK_FROZEN']:
        errors.append('post-Final Navy freeze gate missing')
    if navy.get('phase')!='beta18-final': errors.append('frozen Navy registry phase must remain beta18-final')
    ...
    # RC1 semantic freeze: all Navy gameplay Jomini remains byte-semantically frozen.
    # The historical Final manifest also froze the entire generated Workspace. Post-Final
    # modules need to add independent UI, so that one broad file hash is replaced by a
    # semantic hash of the accepted Navy/Fleet named blocks only.
    semantic_mismatch=[]
    postfinal=build.get('version')!='0.3-beta18-final'
    for rel,expected_hash in freeze.get('semantic_files',{}).items():
        if postfinal and postfreeze and rel=='generated/workspace_shell.gui.txt':
            continue
        p=ROOT/rel
        if not p.exists():
    ...
    workspace_surface={'checked':False}
    if postfinal:
        if not postfreeze:
            errors.append('missing scoped post-Final Navy Workspace freeze manifest')
        else:
            expected=postfreeze.get('workspace_surface',{})
            try:
                block_count,surface_hash=workspace_navy_surface_hash(read('generated/workspace_shell.gui.txt'))
                workspace_surface={'checked':True,'block_count':block_count,'semantic_sha256':surface_hash}
                if block_count!=expected.get('expected_block_count'):
                    errors.append(f'post-Final Navy Workspace block count changed: {block_count}')
                if surface_hash!=expected.get('semantic_sha256'):
                    errors.append('post-Final Navy Workspace surface changed semantically')
            except Exception as exc:
                errors.append('cannot isolate post-Final Navy Workspace surface: '+str(exc))
    raw_mismatch=[]
    ...
    checks.update({
      ...
      'workspace_navy_scoped_freeze':workspace_surface,
